        ...
        for (let row = -fieldSizeHalf; row <= fieldSizeHalf; row += 1) {
            for (let col = -fieldSizeHalf; col <= fieldSizeHalf; col += 1) {
                ...
                if (tile) {
                    ...
                    // count fields
                    if (place === PlaceType.Field) {
                        const { halves } = tile.borders;
                        const checked = new Field(false);
                        checked.set(tile.position.x, tile.position.y, true);
                        ...
                        for (let i = 0; i < halves.length; i += 1) {
                            ...
                            const { x: bx, y: by } = tile.position;
                            const { x: ox, y: oy } = Tile.getFieldOffset(i);
                            const x = bx + ox,
                                y = by + oy;
                            const tileToCheck = this.field.get(x, y);
                            if (!tileToCheck) {
                        ...
                        const closedTownsIds = new Set<number>();
                        ...
                        for (; queuePos < queue.length; queuePos += 1) {
                            ...
                            for (const townId of tileq.closedTownIds) {
                                closedTownsIds.add(townId);
                            }
                            if (tileq.meeple && tileq.meeple.placeId === id) {
                                meeples.push(tileq.meeple);
                            }
                            const { halves } = tileq.borders;
                            for (let i = 0; i < halves.length; i += 1) {
                                if (halves[i] != id) {
                                    continue;
                                }
                        ...
                        meeplesCount
                            .filter((x) => x.count === maxMeeplesCount)
                            .forEach((x) => {
                                const amount = closedTownsIds.size * 3;
                                x.player.scores.fields += amount;
                            });
